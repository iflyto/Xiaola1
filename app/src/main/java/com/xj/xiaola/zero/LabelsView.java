...
public class LabelsView extends ViewGroup implements View.OnClickListener {
    ...
    //保存选中的label的位置
    private ArrayList<Integer> mSelectLabels = new ArrayList<Integer>();

    //保存必选项。在多选模式下，可以设置必选项，必选项默认选中，不能反选
    private ArrayList<Integer> mCompulsorys = new ArrayList<Integer>();
    ...
    public void onClick(View v) {
        if (v instanceof TextView) {
            ...
            if (mLabelClickListener != null) {
                mLabelClickListener.onLabelClick(label, label.getText().toString(), (Integer) v.getTag());
            }
        }
    }
    ...
    public void setCompulsorys(int... positions) {
        if (mSelectType == SelectType.MULTI && positions != null) {
            List<Integer> ps = new ArrayList<Integer>(positions.length);
            for (int i : positions) {
                ps.add(i);
            }
            setCompulsorys(ps);
